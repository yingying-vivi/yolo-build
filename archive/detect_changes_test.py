...
def detect_buildings(model, img_path, imgsz):
...
def draw_comparison(img_a, img_b, boxes_a, boxes_b, matches, new_buildings, disappeared,
    ...
    img_a_thumb = cv2.resize(img_a, (thumb_w, thumb_h))
    ...
    for ba in boxes_a:
        ...
        x1, y1, x2, y2 = [int(v * scale) for v in ba["bbox"]]
        if is_disappeared:
            color = (128, 0, 128)
...
def main():
    model_path = ALL_MODELS[ACTIVE_MODEL]
    if not os.path.exists(model_path):
        print(f"[{ACTIVE_MODEL}] 模型权重不存在: {model_path}, 请先训练该模型")
        return

    output_dir = os.path.join(BASE_OUTPUT, ACTIVE_MODEL)
    ...
    for img_a_name, img_b_name in PAIRS:
        ...
        path_a = os.path.join(TEST_DIR, img_a_name)
        path_b = os.path.join(TEST_DIR, img_b_name)

        print(f"  加载A期影像: {path_a}")
        img_a = read_tiff(path_a)
        print(f"  加载B期影像: {path_b}")
        img_b = read_tiff(path_b)
        print(f"  A期影像尺寸: {img_a.shape}, B期影像尺寸: {img_b.shape}")

        boxes_a = detect_buildings(model, path_a, IMGSZ)
        boxes_b = detect_buildings(model, path_b, IMGSZ)
    ...
    txt_path = os.path.join(output_dir, "detect_report.txt")
