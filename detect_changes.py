...
def draw_comparison(img_a, img_b, boxes_a, boxes_b, new_buildings, save_path):
    ...
    for ba in boxes_a:
        ...
        cv2.putText(canvas, f"A:{ba['class']}", (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
    for bb in boxes_b:
        x1, y1, x2, y2 = [int(v) for v in bb["bbox"]]
        x1 += w
        x2 += w
        color = (0, 0, 255) if bb in new_buildings else (255, 255, 0)
        label = "NEW!" if bb in new_buildings else f"B:{bb['class']}"
...
def main():
    ...
    for img_a_name, img_b_name in PAIRS:
        ...
        save_path = os.path.join(OUTPUT_DIR, f"compare_{img_a_name}_{img_b_name}.png")
        draw_comparison(img_a, img_b, boxes_a, boxes_b, new_buildings, save_path)

        print(f"Period A ({img_a_name}): {len(boxes_a)} buildings")
